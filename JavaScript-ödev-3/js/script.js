...
for (let i = 0; i < menu.length; i++) {
	...
	let menuTitleHeader = document.createElement('h4');
	let menuTitlePrice = document.createElement('h4');

	itemPic.src = `${menu[i].img}`;
	menuTitleHeader.innerHTML = `${menu[i].title}`;
	menuTitlePrice.innerHTML = `${menu[i].price}`;
	menuText.innerHTML = `${menu[i].desc}`;

	divMenuItem.classList.add('menu-items', 'col-lg-6', 'col-sm-12');
	menuInfo.classList.add('menu-info');
	itemPic.classList.add('photo');
	...
	divMenuItem.appendChild(itemPic);
